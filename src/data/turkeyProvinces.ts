...
export type ProvinceCode = (typeof turkeyProvinces)[number]['code'];

const provinceNames = new Map<number, string>(
  turkeyProvinces.map((province) => [province.code, province.name]),
);

export function isProvinceCode(value: unknown): value is ProvinceCode {
  return typeof value === 'number' && Number.isInteger(value) && provinceNames.has(value);
}

export function getProvinceName(code: ProvinceCode): string {
  return provinceNames.get(code) ?? 'Bilinmeyen şehir';
}
