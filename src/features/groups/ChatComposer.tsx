...
const EmojiPanel = memo(function EmojiPanel({ onSelect, recent }: { onSelect: (emoji: string) => void; recent: readonly string[] }) {
  ...
  return <View style={{ backgroundColor: colors.surface, borderTopColor: colors.divider, borderTopWidth: 1, height: 260, paddingHorizontal: spacing.sm }}>
    <FlatList
...
export const ChatComposer = memo(function ChatComposer({ disabled, onAttachment, onSend }: {
  ...
  return <View>
    ...
    {emojiOpen ? <EmojiPanel onSelect={insertEmoji} recent={recentEmojis} /> : null}
  </View>;
});
