...
export const DocumentMessage = memo(function DocumentMessage({ groupId, message, textColor }: {
  ...
  const open = async () => {
    ...
    try {
      ...
      setNotice({ title: 'Belge indirilemedi', description: 'İnternet bağlantını kontrol edip tekrar dene.' });
      setOpening(false);
      return;
    }
    try {
      ...
      setNotice({ title: 'Belge açılamadı', description: 'Bu belgeyi açabilecek bir uygulama bulunamadı.' });
  ...
  </Pressable><DevremNoticeModal description={notice?.description ?? ''} onClose={() => setNotice(null)} title={notice?.title ?? ''} visible={notice !== null} /></>;
});
