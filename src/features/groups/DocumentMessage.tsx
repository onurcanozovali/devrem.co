...
export const DocumentMessage = memo(function DocumentMessage({ groupId, message, textColor }: {
  ...
  const open = async () => {
    ...
    setOpening(true);
    try {
      const uri = message.localMediaUri ?? await resolveChatMediaLocalUri(groupId, message.id, 'document', message.mediaPath, message.extension);
      if (await Sharing.isAvailableAsync()) await Sharing.shareAsync(uri, { dialogTitle: message.fileName, mimeType: message.mimeType });
